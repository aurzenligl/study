...
class PortAllocator(object):
    def __init__(self, dbpath, first):
        self.alloc = Allocator(dbpath)
        self.first = first

    def allocate(self):
        while True:
            port = self.alloc.allocate(self.first)
            if not is_port_free(port):
                continue
            return port

    def allocate_n(self, count):
        return [self.allocate() for _ in range(count)]

    def free(self, port):
        self.alloc.free(port)

    def free_n(self, ports):
        for port in ports:
            self.free(port)
